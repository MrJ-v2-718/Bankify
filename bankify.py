...
def create_account_click():
    ...
    # Write Data To Account Log
    log_file_path = "AccountCreation.txt"
    log_file = open(log_file_path, "a")
    log_file.write(f"Account ID: {account_id}\n")
    log_file.write(f"Name: {name}\n")
    log_file.write(f"Email: {email}\n")
    log_file.write(f"Creation Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    log_file.close()
...
def deposit_click():
    ...
    transaction_file_path = os.path.join(accounts_directory, account_id)
    transaction_file = open(transaction_file_path + "_transactions.txt", "a")
    transaction_file.write(f"Transaction Type : Deposit\n")
    transaction_file.write(f"Transaction Amount : ${"{:.2f}".format(rounded_deposit)}\n")
    transaction_file.write(f"Transaction Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
...
def withdrawal_click():
    ...
    transaction_file_path = os.path.join(accounts_directory, account_id)
    transaction_file = open(transaction_file_path + "_transactions.txt", "a")
    transaction_file.write(f"Transaction Type : Withdrawal\n")
    transaction_file.write(f"Transaction Amount : ${"{:.2f}".format(rounded_withdrawal)}\n")
    transaction_file.write(f"Transaction Time: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n\n")
    transaction_file.close()
...
def balance():
    ...
    personal_info_screen.title("Balance & Account Details")
    ...
    # Open user transactions file and list account activity
    transaction_file_path = os.path.join(accounts_directory, info_account_id)
    transaction_file = transaction_file_path + "_transactions.txt"
    ...
    try:
        with open(transaction_file, "r") as file:
            ...
            # Display each transaction in the text box
            for transaction in transactions:
                text_box.insert(tk.END, transaction)

    except FileNotFoundError:
        text_box.insert(tk.END, "No transaction history.")
